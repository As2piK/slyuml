...
public class HierarchicalView extends JPanelRounded implements IClassComponentsObserver, IDBComponentsObserver, TreeSelectionListener
{
	private final DefaultMutableTreeNode entitiesNode, associationsNode,
			inheritancesNode, dependenciesNode, tableNode, relationNode;
	private final JTree tree;
	private final DefaultTreeModel treeModel;
	final DefaultMutableTreeNode root;
	final DefaultMutableTreeNode classRoot;
	final DefaultMutableTreeNode dbRoot;
	 ...
	 */
	public HierarchicalView(ClassDiagram classDiagram, DBDiagram dbDiagram)
	{
		...
		root = new DefaultMutableTreeNode("Diagrams entities");
		classRoot = new DefaultMutableTreeNode(classDiagram.getName());
		dbRoot = new DefaultMutableTreeNode(dbDiagram.getName());

		entitiesNode = new DefaultMutableTreeNode("Entities");
		classRoot.add(entitiesNode);

		associationsNode = new DefaultMutableTreeNode("Relations");
		classRoot.add(associationsNode);

		inheritancesNode = new DefaultMutableTreeNode("Inheritances");
		classRoot.add(inheritancesNode);

		dependenciesNode = new DefaultMutableTreeNode("Dependencies");
		classRoot.add(dependenciesNode);

		tableNode = new DefaultMutableTreeNode("Tables");
		dbRoot.add(tableNode);
		
		relationNode = new DefaultMutableTreeNode("Relations");
		dbRoot.add(relationNode);
	...
	public void switchVisibleNodes() {
		if (Slyum.currentDiagramType == DIAGRAM_TYPE.CLASS) {
			treeModel.setRoot(classRoot);
		} else {
			treeModel.setRoot(dbRoot);
		}
	}
}
