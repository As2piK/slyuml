...
public class GraphicView extends GraphicComponent implements MouseMotionListener, MouseListener, IClassComponentsObserver, Printable, KeyListener, MouseWheelListener
{
	...
	private CreateGenericComponent currentFactory;

	private final LinkedList<AbstractEntityView> entities = new LinkedList<>();
	...
	public void addDependency(Dependency component)
	{
		...
		if (result == null)
		{
			final ClassEntityView source = (ClassEntityView) searchAssociedComponent(component.getSource());
			final ClassEntityView target = (ClassEntityView) searchAssociedComponent(component.getTarget());
	 ...
	 */
	public boolean addEntity(AbstractEntityView component)
	{
	...
	public void addInheritance(Inheritance component)
	{
		...
		if (result == null)
		{
			final ClassEntityView child = (ClassEntityView) searchAssociedComponent(component.getChild());
			final ClassEntityView parent = (ClassEntityView) searchAssociedComponent(component.getParent());
	 ...
	 */
	public void adjustEntities(LinkedList<AbstractEntityView> list)
	{
		boolean isRecord = Change.isRecord();
		Change.record();
		
		for (final AbstractEntityView ev : list)
	...
	{		
		...
		final LinkedList<AbstractEntityView> sorted = sortXLocation(getSelectedEntities());

		if (sorted.size() < 2)
			return;
		
		for (final AbstractEntityView c : sorted)
		{
	...
	public void alignVertical(boolean left)
	{
		...
		final LinkedList<AbstractEntityView> sorted = sortYLocation(getSelectedEntities());
	...
	{
		final ClassEntityView firstView = (ClassEntityView) searchAssociedComponent(first);
	...
	public LinkedList<AbstractEntityView> getEntitiesView()
	{
		return (LinkedList<AbstractEntityView>) entities.clone();
	}
	 ...
	 */
	public AbstractEntityView getEntityAtPosition(Point pos)
	{
	 ...
	 */
	public LinkedList<AbstractEntityView> getSelectedEntities()
	{
		final LinkedList<AbstractEntityView> selectedEntities = new LinkedList<AbstractEntityView>();
	 ...
	 *            the new factory
	 */
	public void initNewComponent(CreateGenericComponent factory)
	{
		if (factory == null)
	...
	{
		LinkedList<AbstractEntityView> evs = getSelectedEntities();
		LinkedList<AbstractEntityView> evsSorted = new LinkedList<>();
	...
	public void setZoom(float zoom)
	{
		...
		for (final AbstractEntityView entity : getEntitiesView())
	...
	public void showMethodsForSelectedEntity(boolean show)
	{
		for (final AbstractEntityView ev : getSelectedEntities())
			...
			if (ev instanceof ClassEntityView) {
				((ClassEntityView)ev).setDisplayMethods(show);
			}
	}
	 ...
	 * @return the list sorted
	 */
	public LinkedList<AbstractEntityView> sortYLocation(LinkedList<AbstractEntityView> list)
	{
