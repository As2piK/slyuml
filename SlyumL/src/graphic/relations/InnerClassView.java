...
public class InnerClassView extends InheritanceView
{
	...
	public static void paintExtremity(Graphics2D g2, Point source, Point target, Color borderColor)
	{
		...
		final int vectorX = target.x - (int) (Math.cos(alpha) * (length - 10.0) + source.x);
		final int vectorY = target.y - (int) (Math.sin(alpha) * (length - 10.0) + source.y);
	...
	public InnerClassView(GraphicView parent, EntityView source, EntityView target, Inheritance inheritance, Point posSource, Point posTarget, boolean checkRecursivity)
	{
