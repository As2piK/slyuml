...
import graphic.textbox.TextBoxDBEntityName;
import graphic.textbox.TextBoxEntityName;
import graphic.textbox.TextBoxField;
 ...
 */
public abstract class DBEntityView extends MovableComponent implements Observer
{
	...
	protected LinkedList<TextBoxField> fieldsView = new LinkedList<TextBoxField>();
	...
	private final TextBoxDBEntityName entityName;
	...
	public DBEntityView(final ClassGraphicView parent, Entity component)
	{
		...
		entityName = new TextBoxDBEntityName(parent, component);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if ("AddField".equals(e.getActionCommand()))
			addField();
		...
		else if (Slyum.ACTION_TEXTBOX_UP.equals(e.getActionCommand()) || Slyum.ACTION_TEXTBOX_DOWN.equals(e.getActionCommand()))
		{
			...
			if (pressedTextBox.getClass() == TextBoxField.class)
			{
				final Field field = (Field) ((TextBoxField) pressedTextBox).getAssociedComponent();
	...
	/**
	 * Create a new field with default type and name.
	 */
	public void addField()
	{
		final Field field = new Field("field", PrimitiveType.INTEGER_TYPE);
		prepareNewField(field);
	...
	{
		final TextBoxField newTextBox = new TextBoxField(parent, field);
		fieldsView.add(newTextBox);
	...
	public int computeHeight(int classNameHeight, int stereotypeHeight, int elementsHeight)
	{
		...
		if (displayAttributes)
			height += elementsHeight * fieldsView.size();
	...
	public void maybeShowPopup(MouseEvent e, JPopupMenu popupMenu)
	{
		if (e.isPopupTrigger())
		{
			...
			if (pressedTextBox != null)
			{
				...
				menuItemMoveUp.setEnabled(fieldsView.indexOf(pressedTextBox) != 0 && methodsView.indexOf(pressedTextBox) != 0);
				menuItemMoveDown.setEnabled((fieldsView.size() == 0 || fieldsView.indexOf(pressedTextBox) != fieldsView.size() - 1) && (methodsView.size() == 0 || methodsView.indexOf(pressedTextBox) != methodsView.size() - 1));
			}
			else
			{
	...
	private void methodViewChangeClicked(ParametersViewStyle newStyle)
	{
		if (pressedTextBox instanceof TextBoxMethod)
			...
			for (final DBEntityView ev : parent.getSelectedEntities())
	...
	public void paintComponent(Graphics2D g2)
	{
		...
		// draw attributes
		if (displayAttributes && fieldsView.size() > 0)
			// draw methods
			for (final TextBoxAttribute tb : fieldsView)
			{
	 ...
	 */
	protected abstract void prepareNewField(Field field);
	 ...
	 */
	public boolean removeField(TextBoxField tbField)
	{
		if (component.removeField((Field) tbField.getAssociedComponent()))
		{
			component.notifyObservers();
